...
use ethers::{prelude::abigen, providers::Middleware, types::{Address, TransactionReceipt}, contract::ContractError};
...
pub trait OperatorTrait<M: Middleware> {
    async fn register_operator(&self, new_operator: Address) -> Result<Option<TransactionReceipt>, Error<M>>;
    async fn unregister_operator(&self, removed_operator: Address) -> Result<Option<TransactionReceipt>, Error<M>>;
